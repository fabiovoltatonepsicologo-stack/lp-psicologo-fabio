...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // ---- Recalcula altura do acordeao aberto ao redimensionar ----
    let resizeTimer;
    ...
    // ---- Rastreamento de clique no WhatsApp (a conversao real do site) ----
    // So dispara se o gtag.js do GA4 estiver carregado (silencioso se bloqueado por adblock)
    document.querySelectorAll('a[href*="wa.me"]').forEach((link) => {
        link.addEventListener("click", () => {
            if (typeof gtag === "function") {
                gtag("event", "whatsapp_click", {
                    event_category: "contato",
                    event_label: link.closest("section, header, .whatsapp-float")?.className || "desconhecido",
                });
            }
        });
    });
